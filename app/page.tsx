...
const coldFoamOptions: ExtraOption[] = [
  { name: "No Cold Foam", price: 0 },
  { name: "Add Cold Foam", price: 0.5 },
];

const menuItems: MenuItem[] = [
  { id: "espresso", name: "Espresso", category: "Espresso Favorites", price: 1.75 },
  { id: "americano-regular", name: "Regular Americano", category: "Espresso Favorites", price: 3.5 },
  ...
  { id: "red-eye-regular", name: "Regular Red Eye", category: "Espresso Favorites", price: 3.5 },
  { id: "red-eye-large", name: "Large Red Eye", category: "Espresso Favorites", price: 5.0 },
  {
    id: "mocha-regular",
    name: "Regular Mocha",
    category: "Espresso Favorites",
  ...
  { id: "tea", name: "Tea", category: "Other Drinks", price: 3.65 },
  {
    id: "hot-chocolate-regular",
  ...
  { id: "lemonade-regular", name: "Regular Lemonade", category: "Other Drinks", price: 3.75 },
  { id: "lemonade-large", name: "Large Lemonade", category: "Other Drinks", price: 4.25 },

  {
    id: "frappe-mocha",
    name: "16 oz Mocha Frappe",
    category: "Frappes",
    price: 6.75,
    allowsColdFoam: true,
  },
...
export default function Page() {
  ...
  const [selectedOptions, setSelectedOptions] = useState<
    Record<string, { milk: string; flavor: string; coldFoam: string; notes: string }>
  >({});
  ...
  const addToCart = (item: MenuItem) => {
    ...
    const selectedColdFoam = coldFoamOptions.find((foam) => foam.name === savedOptions.coldFoam);
    ...
    const newItem: CartItem = {
      ...
      coldFoam: item.allowsColdFoam ? savedOptions.coldFoam : undefined,
      coldFoamUpcharge: item.allowsColdFoam ? selectedColdFoam?.price || 0 : 0,
  ...
  const updateSelectedOption = (
    itemId: string,
    field: "milk" | "flavor" | "coldFoam" | "notes",
    value: string
  ) => {
    setSelectedOptions((prev) => ({
      ...
      [itemId]: {
        ...
        coldFoam: prev[itemId]?.coldFoam || "No Cold Foam",
  ...
  const submitOrder = () => {
    ...
    cart.forEach((item, index) => {
      lines.push(
        `${index + 1}. ${item.quantity} x ${item.baseName} - ${formatCurrency(item.basePrice)}`
      );
      if (item.milk) lines.push(`   Milk: ${item.milk}`);
      if (item.flavor && item.flavor !== "None") lines.push(`   Flavor: ${item.flavor}`);
      if (item.coldFoam && item.coldFoam !== "No Cold Foam") lines.push(`   Cold Foam: Yes`);
          ...
          <section className="rounded-3xl bg-white p-5 shadow-xl md:p-6">
            <div className="mb-6 rounded-3xl bg-gradient-to-r from-amber-900 via-amber-800 to-stone-800 p-6 text-white shadow-lg">
              ...
              <p className="mt-2 text-sm text-amber-100">
                Whole milk included. Oat milk and almond milk are $1.00 extra. Cold foam is $0.50.
              </p>
            </div>
                          ...
                          <div className="space-y-3">
                            ...
                            {item.allowsColdFoam ? (
                              <div>
                                ...
                                <select
                                  className="w-full rounded-xl border border-stone-300 bg-white px-3 py-2 text-sm"
                                  value={optionState.coldFoam}
                                  onChange={(e) =>
                                    updateSelectedOption(item.id, "coldFoam", e.target.value)
                                  }
                                >
                                  {coldFoamOptions.map((foam) => (
                                    <option key={foam.name} value={foam.name}>
                                      {foam.name}
                                      {foam.price > 0 ? ` (+${formatCurrency(foam.price)})` : ""}
                                    </option>
                                  ))}
                                </select>
                              </div>
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          {item.coldFoam && item.coldFoam !== "No Cold Foam" ? (
                            <p className="text-xs text-stone-600">Cold Foam: Yes</p>
                          ) : null}
                          {item.notes ? (
